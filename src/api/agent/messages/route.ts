...
interface AgentMessagePayload {
  ...
  instanceName?: string; // Optional: specify instance to use
...
export async function POST(request: Request) {
  ...
  try {
    ...
    let { chatId, phoneNumber, content = '', media, instanceName } = body;
    ...
    if (media && media.length > 0) {
        result = await sendAutomatedMediaAction(chatId, content, media, agent.id, instanceName);
    } 
    // Otherwise, send a text message.
    else {
        result = await sendAutomatedMessageAction(chatId, content, agent.id, true, instanceName);
    }
